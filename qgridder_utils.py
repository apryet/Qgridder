...
def lin_time_interp(df_in, t_out, col_name = 'val') : 
    """
    ...
    Parameters
    ----------
    df_in : Pandas DataFrame object with a date index and a 'val' column
    t_out : dates for which to provide interpolated values.
    col_name : name column of df_in containing values of interest
    
    Returns
    -------

    df_out : Pandas DataFrame object with interpolated values

    Examples
    --------
    >>> df_in = pd.read_csv('./data_in.dat')
    >>> df_in['date'] = pd.to_datetime@( df_in['TIMESTAMP'])
    >>> df_in = df_in.set_index('date')
    >>> t_out = pd.date_range('2015-03-02 12:00', periods=6, freq='5 min')
    >>> df_out = lin_time_interp(df_in, t_out)
	...
	print("Cannot load python module \"pandas\" ")
	return(None)
	
    # set up output data frame
    df_out = pd.DataFrame(index=t_out)

    # array of indexes corresponding with closest timestamp after "observed" value 
    idx_after = np.searchsorted(df_in.index.values, df_out.index.values)

    # look for dates outside time span of input values
    df_out['outside'] = 0 # default value, for dates within time span of input values
    df_out['outside'][ idx_after == 0 ] = -1 # value for dates before first input value
    df_out['outside'][ idx_after == len(df_in.index) ] = 1 # value for dates after last input value
    idx_after[ idx_after == 0 ] = 1 # correct idx_after to avoid index value out of bound 
    idx_after[ idx_after == len(df_in.index) ] = len(df_in.index) - 1 # correct idx_after to avoid index value out of bound

    # values and timestamp before/after resample
    df_out['before_time'] = df_in.index[idx_after - 1]
    df_out['after_time'] = df_in.index[idx_after]

    df_out['before_val'] = df_in.loc[df_in.index[idx_after - 1], col_name].values
    df_out['after_val'] = df_in.loc[df_in.index[idx_after], col_name].values

    #calculate new weighted value
    df_out['span'] = (df_out['after_time'] - df_out['before_time'])
    df_out['before_weight'] = (df_out['after_time'] - df_out.index ) / df_out['span']
    df_out['after_weight'] = (df_out.index - df_out['before_time'] ) / df_out['span']
    df_out[col_name] = df_out['before_weight'] * df_out['before_val'] + df_out['after_weight']*df_out['after_val']

    # set values outside to first / last value
    df_out.loc[ df_out['outside'].values == -1, col_name ] = df_in[col_name][0]
    df_out.loc[ df_out['outside'].values == 1, col_name ] = df_in[col_name][-1]

    return(df_out[[col_name]])



def interp_from_file(file_path, t_out, col_name = 'val') : 
    """
    Perform linear interpolation from an input csv file with lin_time_interp

    Parameters
    ----------
    file_in : input csv file following format 
		TIMESTAMP,val
    ...
    # compute df_out
    df_out = lin_time_interp(df_in, t_out, col_name)
    return( df_out ) 
